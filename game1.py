...
def get_weather():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result = (f"\U0001F30D City: {city.capitalize()}\n"
                      f"\U0001F321️ Temperature: {temperature}°C\n"
                      f"\U0001F4A7 Humidity: {humidity}%\n"
                      f"\U0001F343 Wind Speed: {wind_speed} m/s\n"
                      f"\U0001F324️ Description: {description}")
...
app = Tk()
app.title("Exciting Weather App \U0001F31F")
app.geometry("800x800")
app.resizable(False, False)

# Set Background Image
background_img_path = resource_path("weather.png")  # Replace with your image path
try:
    bg_img = PhotoImage(file=background_img_path)
...
header_label = Label(header_frame, text="\U0001F324️ Exciting Weather App", font=("Helvetica", 20, "bold"), fg="white", bg="#8bc34a")
header_label.pack(pady=10)

# Input Frame
input_frame = Frame(app, bg="white")
input_frame.place(relx=0.5, rely=0.3, anchor="center")

city_label = Label(input_frame, text="Enter City:", font=("Helvetica", 14), bg="white")
...
# Icon Label
icon_label = Label(app, bg="white")
icon_label.place(relx=0.5, rely=0.4, anchor="center")

# Result Frame
result_frame = Frame(app, bg="white")
...
# Graph Frame
graph_frame = Frame(app, bg="white")
...
# footer_label = Label(footer_frame, text="Powered by OpenWeatherMap \U0001F30D", font=("Helvetica", 10), fg="white", bg="#8bc34a")
# footer_label.pack()

# Buttons
button_frame = Frame(app, bg="white")
button_frame.place(relx=0.5, rely=0.6, anchor="center")

fetch_button = Button(button_frame, text="Get Weather \U0001F3AF", font=("Helvetica", 14), bg="#4caf50", fg="white", relief="raised", command=get_weather)
fetch_button.grid(row=0, column=0, padx=10)

clear_button = Button(button_frame, text="Clear \U0001F5D1️", font=("Helvetica", 14), bg="#f44336", fg="white", relief="raised", command=clear_weather)
